...
source "qemu" "onprem" {
  ...
  output_directory = "images/erid-nomad-${var.image_version}"
  vm_name          = "erid-nomad-${var.image_version}.qcow2"
  disk_size        = "5G"
  format           = "qcow2"
  accelerator      = "kvm"
  ssh_username     = "erid"
...
source "amazon-ebs" "cloud-x86" {
  ...
  ami_name      = "erid-nomad-${var.image_version}-{{timestamp}}.x86"
...
source "amazon-ebs" "cloud-arm" {
  ...
  ssh_username  = "ec2-user"
  ami_name      = "erid-nomad-${var.image_version}-{{timestamp}}.arm"
...
build {
  ...
  provisioner "shell" {
    inline = [
      "sudo dnf update -y",
      "sudo dnf install -y yum-utils curl git cloud-init",
